    ...
    size_t      vmm_budget_bytes)
{
    ...
    if (hdr->magic != VMM_MODEL_MAGIC) {
        fprintf(stderr, "[VMM] bad magic in vmm.bin\n");
    ...
    size_t      vmm_budget_bytes)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (vmm_alloc(vmm, td->size_bytes, 0, -1, &r) != 0) {
            fprintf(stderr, "[VMM] data alloc FAILED i=%d\n", i);
            fclose(f);
            vmm_destroy(vmm);
            free(tensors);
            gguf_free(ctx);
        ...
        if (fseek(f, (long)td->file_offset, SEEK_SET) != 0 ||
            ...
            fprintf(stderr, "[VMM] fread FAILED i=%d\n", i);
            fclose(f);
            vmm_destroy(vmm);
            free(tensors);
            gguf_free(ctx);
    ...
    fprintf(stderr, "[VMM] build_from_gguf OK → %s\n", vmm_path);
    return 0;
}
